...
const online = { roomId: null, code: null, isHost: false, unsub: null, applying: false };
...
function subscribeRoom(){online.unsub?.();if(!online.roomId)return;online.unsub=onSnapshot(doc(db,"rooms",online.roomId),snap=>{if(snap.exists())applySnapshot(snap.data().snapshot)})}
async function createOnlineRoom(){online.isHost=true;online.roomId=crypto.randomUUID();for(let tries=0;tries<8;tries++){online.code=makeRoomCode();if(!(await getDoc(doc(db,"roomCodes",online.code))).exists())break}state.revision=1;const snapshot=publicSnapshot();const batch=writeBatch(db);batch.set(doc(db,"rooms",online.roomId),{teacherId:auth.currentUser.uid,code:online.code,status:"playing",snapshot,createdAt:serverTimestamp(),updatedAt:serverTimestamp()});batch.set(doc(db,"roomCodes",online.code),{roomId:online.roomId,active:true,createdAt:serverTimestamp()});batch.set(doc(db,"rooms",online.roomId,"players",auth.currentUser.uid),{role:"teacher",teamIndex:-1,joinedAt:serverTimestamp()});await batch.commit();subscribeRoom();updateOnlineBadges();try{await loadQuizBank()}catch(error){console.error("Quiz bank load after room creation failed",error)}}
async function joinOnlineRoom(){const errorBox=$("#join-error"),code=$("#join-code").value.trim().toUpperCase(),name=$("#join-team-name").value.trim();if(code.length!==6){errorBox.textContent="6자리 입장 코드를 입력하세요.";return}errorBox.textContent="입장 중입니다…";try{if(!auth.currentUser)await signInAnonymously(auth);const codeSnap=await getDoc(doc(db,"roomCodes",code));if(!codeSnap.exists()||!codeSnap.data().active)throw new Error("사용할 수 없는 입장 코드입니다.");const roomId=codeSnap.data().roomId,roomRef=doc(db,"rooms",roomId),playerRef=doc(db,"rooms",roomId,"players",auth.currentUser.uid);let joinedIndex=-1;await runTransaction(db,async tx=>{const roomSnap=await tx.get(roomRef);if(!roomSnap.exists())throw new Error("게임방을 찾을 수 없습니다.");const data=roomSnap.data(),snapshot=data.snapshot,teams=snapshot.teams.map(t=>({...t}));joinedIndex=teams.findIndex(t=>t.ownerUid===auth.currentUser.uid);if(joinedIndex<0)joinedIndex=teams.findIndex(t=>!t.ownerUid);if(joinedIndex<0)throw new Error("모든 팀 자리가 찼습니다.");teams[joinedIndex].ownerUid=auth.currentUser.uid;if(name)teams[joinedIndex].name=name;const next={...snapshot,teams,revision:(snapshot.revision||0)+1};tx.set(playerRef,{role:"student",teamIndex:joinedIndex,teamName:teams[joinedIndex].name,joinedAt:serverTimestamp()});tx.update(roomRef,{snapshot:next,updatedAt:serverTimestamp()})});online.roomId=roomId;online.code=code;online.isHost=false;state.revision=0;subscribeRoom();await loadQuizBank();$("#join-dialog").close();updateOnlineBadges()}catch(error){errorBox.textContent=error.message}}
...
$("#student-entry").onclick=()=>{$("#join-code").value="";$("#join-team-name").value="";$("#join-error").textContent="";$("#join-dialog").showModal()};$("#teacher-entry").onclick=teacherLogin;$$('.back-home').forEach(b=>b.onclick=()=>showScreen("home-screen"));$("#start-game").onclick=async()=>{try{resetGame();await createOnlineRoom();showScreen("game-screen");showRoomAccess()}catch(error){alert(`게임방 생성 실패: ${error.message}`)}};$("#teacher-start").onclick=()=>{online.isHost=true;setupTeams(4);showScreen("setup-screen")};$("#room-access-button").onclick=showRoomAccess;$("#copy-join-link").onclick=copyJoinLink;$("#exit-game").onclick=()=>showScreen("home-screen");$("#open-report").onclick=renderReport;$("#winner-report").onclick=()=>{$("#winner-dialog").close();renderReport()};
function openTv(){window.open(`${location.pathname}#tv`,"history-tv","noopener")};$("#tv-button").onclick=openTv;$("#teacher-tv").onclick=openTv;
const channel=new BroadcastChannel("history-exploration");async function syncState(){const safe={teams:state.teams.map(({name,token,color,position})=>({name,token,color,position})),current:state.current,lastRoll:state.lastRoll};localStorage.setItem("history-game-state",JSON.stringify(safe));channel.postMessage(safe);if(online.roomId&&!online.applying){state.revision++;try{await updateDoc(doc(db,"rooms",online.roomId),{snapshot:publicSnapshot(),updatedAt:serverTimestamp()})}catch(error){console.error("Firestore sync failed",error)}}}channel.onmessage=e=>{if(location.hash==="#tv")applyTv(e.data)};function applyTv(data){if(!data?.teams)return;state.teams=data.teams;state.current=data.current;state.lastRoll=data.lastRoll;renderTokens()}if(location.hash==="#tv"){showScreen("tv-screen");try{applyTv(JSON.parse(localStorage.getItem("history-game-state")))}catch{}}else showScreen("home-screen");
...
async function loadQuizBank(){if(!auth.currentUser)return;const publicDocs=await getDocs(query(collection(db,"quizPublic"),limit(500)));const items=publicDocs.docs.map(d=>({id:d.id,...d.data()})).filter(q=>q.active!==false).sort((a,b)=>(a.order||0)-(b.order||0));if(auth.currentUser.email?.toLowerCase()===TEACHER_EMAIL){const answerDocs=await getDocs(query(collection(db,"quizAnswers"),limit(500))),answers=new Map(answerDocs.docs.map(d=>[d.id,d.data().answer]));items.forEach(q=>q.answer=answers.get(q.id))}state.quizzes=items;$("#question-count").textContent=items.length;return items}
...
if(location.hash!=="#tv"&&/^[A-HJ-NP-Z2-9]{6}$/.test(sharedJoinCode||"")){$("#join-code").value=sharedJoinCode;$("#join-error").textContent="QR 접속 코드가 자동 입력되었습니다. 팀 이름을 입력하세요.";requestAnimationFrame(()=>$("#join-dialog").showModal())}
